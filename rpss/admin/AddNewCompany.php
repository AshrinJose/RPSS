...
<h1 class="title">Add New <span>Company</span></h1>
</div>
...
<?php
	...
	$start_from=($page-1)*1;
	$sql="select * from tbl_personaldetails,tbl_login where tbl_login.status=0 and tbl_personaldetails.type=3 and tbl_personaldetails.code_no=tbl_login.code_no limit $start_from,1";
...
<?php
	$sql="select count(status) from tbl_login where tbl_login.type=3 and tbl_login.status=0";
	$result=mysql_query($sql);
	...
	{
		echo "<a href='AddNewCompany.php?page=".$i."'>".$i."</a>";
	}
	echo "</td></tr>";
	echo "</table>";
?>	
...
{
$c=mysql_query("select * from tbl_login where tbl_login.status=0 and tbl_login.type=3");
if($c>0)
{
...
$sql=mysql_query("update tbl_login set tbl_login.status=1 where tbl_login.username='$username' and tbl_login.type=3");
echo $sql;
if($sql>0)
{
	echo "<script>alert('Company Added..');</script>";
    echo '<META HTTP-EQUIV="Refresh" Content="0; URL=AddNewCompany.php">';
}
else 
	echo mysql_error();
}
